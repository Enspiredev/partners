...
<table class="form-table <?php if($theme_version < 3) { echo "disabled"; } ?>">
    <tbody>
        ...
        <tr>
            <td><strong><input id="taskrocket_settings[clients_edit_tasks]" name="taskrocket_settings[clients_edit_tasks]" type="checkbox" value="1" <?php checked( '1', $options['clients_edit_tasks'] ); ?> /> <?php _e( 'Let clients edit tasks' ); ?></strong>
                <label for="taskrocket_settings[clients_edit_tasks]"><?php _e( 'Let clients edit their own tasks on the front-end.' ); ?></label>
            </td>
        </tr>
        <tr>
            <td>
