...
// Change the author metabox label
add_action('add_meta_boxes', 'change_meta_box_title', 999);
function change_meta_box_title() {
    global $wp_meta_boxes; // array of defined meta boxes
    // cycle through the array, change the titles you want
    
    $to = __( 'Task Owner', 'taskrocket-task-groups');

    $wp_meta_boxes['task_groups']['normal']['core']['authordiv']['title'] = $to;
}
...
add_filter('wp_dropdown_users', 'task_groups_users');
function task_groups_users($output)  {
global $post;

//Doing it only for the custom post type
if($post->post_type == ('task_groups')) {
    // $users = get_users(array('role'=>'editor'));
    $users = get_users(array());
    $task_owner_ID = $post->post_author;

    // debug $output .= $task_owner_ID;
    $output .= "<select id='tr_post_author_override' name='post_author_override' class=''>";
    foreach($users as $user) {
        ...
        $output .= $selected;
        $output .= ">" . $user->user_firstname . ' ' . $user->user_lastname . " (" . $user->user_email . ")</option>";
    }
    if($task_owner_ID == "0") {
    $output .= '<option value="0000000" selected="selected">' . __( 'Nobody (no owner)', 'taskrocket-task-groups' ) . '</option>';
    } else {
        $output .= '<option value="0000000">' . __( 'Nobody (no owner)', 'taskrocket-task-groups' ) . '</option>';
    }
    $output .= "</select>";
}
return $output;
